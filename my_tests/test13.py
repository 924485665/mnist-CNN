...
if __name__ == '__main__':
    ...
    dead_female_surname = list(set(combine_df[(combine_df.Sex == 'female') & (combine_df.Age >= 12)
                                              & (combine_df.Survived == 0) & (
                                              (combine_df.Parch > 0) | (combine_df.SibSp > 0))]['Surname'].values))
    survive_male_surname = list(set(combine_df[(combine_df.Sex == 'male') & (combine_df.Age >= 12)
                                               & (combine_df.Survived == 1) & (
                                               (combine_df.Parch > 0) | (combine_df.SibSp > 0))]['Surname'].values))
    ...
    copy_df = copy.copy(combine_df)
    print(combine_df.columns.values)
    print(copy_df.columns.values)
    copy_df['new_cols'] = combine_df['Survived']
    print(combine_df.columns.values)
    print(copy_df.columns.values)
    ...
    #         "Survived": Y_test
    #     })
    # submission.to_csv(r'D:/titanic/results/bag.csv', index=False)
    #
